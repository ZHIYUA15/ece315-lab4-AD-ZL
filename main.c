...
int main()
{
	...
	xTaskCreate(_Task_Motor, "Motor Task", configMINIMAL_STACK_SIZE, NULL, DEFAULT_THREAD_PRIO + 1, &motorTaskHandle);

	xTaskCreate(pushbutton_task, "PushButtonTask", THREAD_STACKSIZE, NULL, DEFAULT_THREAD_PRIO, NULL);

	// Create the emergency stop task
	xTaskCreate(emergency_stop_task, "EmergencyStopTask", THREAD_STACKSIZE, NULL, DEFAULT_THREAD_PRIO, NULL);

	// Create the LED animation task
	xTaskCreate(led_animation_task, "LEDAnimationTask", THREAD_STACKSIZE, NULL, DEFAULT_THREAD_PRIO, NULL);

	sys_thread_new("main_thrd", (void (*)(void *))main_thread, 0, THREAD_STACKSIZE, DEFAULT_THREAD_PRIO + 1);
